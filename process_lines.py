...
def assemble_phrases(line, qa_phrases, objection_phrases, non_party_phrases, capitalize,
                     phrase_being_assembled):
    completed_line_groups = []

    if any(line.startswith(phrase) for phrase in qa_phrases):
        if phrase_being_assembled and not capitalize:
...
def format_output(completed_line_groups, first_num, last_num, show_depo_name, name):
